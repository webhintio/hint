...
const main = async () => {
    ...
    /*
     * `install` step in Travis does a `yarn`. If versions
     * are different, yarn.lock is different so discard changes.
     */
    await exec(`Discard 'yarn.lock'.`, `git checkout yarn.lock`);

    /*
     * Greenkeeper updates each package in the monorepo with
     * the same dependency in different commits: squash and
     * update message.
     */
