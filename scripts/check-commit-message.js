...
const isExcludedCommit = (commit) => {
    ...
    if ((/^🚀 (hint|(configuration|connector|formatter|parser|rule)(-[0-9a-z]+)+) - v\d+\.\d+\.\d+/i).test(commit.message) ||
        (/^(Chore|Breaking): Update `(hint|(configuration|connector|formatter|parser|rule)(-[0-9a-z]+)+)` to `v\d+\.\d+\.\d+`/i).test(commit.message)) {
        return true;
    }
