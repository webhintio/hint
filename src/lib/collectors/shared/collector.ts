...
import {
    ...
    IElementFound, IFetchEnd, IFetchError, ILauncher, IManifestFetchEnd, IManifestFetchError, ITraverseUp, ITraverseDown,
...
export class Collector implements ICollector {
    ...
    private getClient(): Promise<object> {
        let retries = 0;
        const loadCDP = async () => {
            try {
    ...
    private async initiateComms() {
        ...
        if (newBrowser) {
            ...
            this._tabs = await cdp.List(); //eslint-disable-line new-cap
        } else {
            const tab = await cdp.New(); //eslint-disable-line new-cap
