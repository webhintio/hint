...
test(`'${parseErrorSchemaEventName}' event is emitted when manifest content is not valid because of an additional property`, async (t) => {
    const expectedPrettifiedErrors = [
        `'root' must NOT have additional properties. Additional property found 'additionalProperty'.`,
        `'root' must NOT have additional properties. Additional property found 'gcm_sender_id'.`,
        `'root' must NOT have additional properties. Additional property found 'unknown_proprietary_extension'.`,
        `'icons/0' must NOT have additional properties. Additional property found 'density'.`
    ];
