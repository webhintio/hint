...
${severityToColor(Severity.warning).bold(`${symbols.error.trim()} Found a total of 0 errors, 2 warnings, 0 hints and 0 informations`)}`;
...
test(`Summary formatter prints a table and a summary for all resources combined`, (t) => {
    ...
    const expectedResult = `${table(tableData)}
${severityToColor(Severity.error).bold(`${symbols.error.trim()} Found a total of 1 error, 2 warnings, 1 hint and 1 information`)}`;
...
test(`Summary formatter sorts by name if same number of errors`, (t) => {
    ...
    const expectedResult = `${table(tableData)}
${severityToColor(Severity.error).bold(`${symbols.error.trim()} Found a total of 2 errors, 0 warnings, 0 hints and 0 informations`)}`;
...
test(`Summary formatter prints errors and warnings for a hint that reports both`, (t) => {
    ...
    const expectedResult = `${table(tableData)}
${severityToColor(Severity.error).bold(`${symbols.error.trim()} Found a total of 1 error, 1 warning, 0 hints and 0 informations`)}`;
...
test(`Summary formatter called with the output option should write the result in the output file`, (t) => {
    ...
    const expectedResult = `${table(tableData)}
${stripAnsi(symbols.error.trim())} Found a total of 1 error, 1 warning, 0 hints and 0 informations`;
