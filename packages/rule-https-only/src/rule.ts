/**
 * @fileoverview sonarwhal rule that checks if your site is using HTTPS and if it has mixed content.
 */
...
import { IRule, RuleMetadata, FetchEnd, Response, ElementFound } from 'hint/dist/src/lib/types';
...
export default class HttpsOnlyRule implements IRule {
    private targetIsServedOverHTTPS: boolean = false;

    public static readonly meta: RuleMetadata = {
        docs: {
            category: Category.security,
            description: `sonarwhal rule that checks if your site is using HTTPS and if it has mixed content.`
        },
        id: 'https-only',
        schema: [],
    ...
    public constructor(context: RuleContext) {
         ...
         * video but `sonarwhal` will still flag that the `ogv` video is served over HTTP
