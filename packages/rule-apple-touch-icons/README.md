...
`apple-touch-icons` warns against using multiple PNG images when
a single `180×180px` PNG image for the `apple-touch-icon` is recommended.

## Why is this important?

Safari for iOS has supported a way for developers to specify an
image that will be used to represent the web site/app on the homescreen
since `iOS 1.1.3`. The image is known as the [touch icon][touch icons]).

Newer progressive web apps approach this via the [web app manifest file][web app
manifest spec], but at this time, Safari for iOS doesn’t support
that method.

Over time as Apple released different size displays for their
devices, developers tended to ship a variety of icon sizes
to cater for multiple device including:
...
The images were sent to all devices regardless of the single icon
best suited for that device.

Declaring just one `180×180px` PNG image, e.g.:
...
Including all the different sizes is not recommended as:
