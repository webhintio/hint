...
test('If no file is parsed, it should emit a `parser::babel-config::error::not-found` error', async (t) => {
    ...
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::babel-config::error::not-found');

    sandbox.restore();
});

test(`If a 'package.json' file is parsed, but it doesn't have the 'babel' property, it should emit a 'parser::babel-config::error::not-found' error`, async (t) => {
    const sandbox = sinon.sandbox.create();
    ...
    t.is(t.context.sonarwhal.emitAsync.args[2][0], 'parser::babel-config::error::not-found');
...
test('If the file contains an invalid json, it should fail', async (t) => {
    ...
    t.true(t.context.sonarwhal.emitAsync.calledTwice);
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::babel-config::error::json');

    sandbox.restore();
});

test(`If .babelrc contains an invalid schema, it should emit the 'parser::babel-config::error::schema' event`, async (t) => {
    const sandbox = sinon.sandbox.create();
    ...
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::babel-config::error::schema');

    sandbox.restore();
});

test(`If 'package.json' contains an invalid 'babel' property, it should emit the 'parser::babel-config::error::schema' event`, async (t) => {
    const sandbox = sinon.sandbox.create();
    const invalidSchemaContent = `{
        "babel": {
          "plugins": ["transform-react-jsx"],
