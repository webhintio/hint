...
const ruleName = 'js-minify-check';

const expectedMessageFromRule = 'Javascript content can be minified';
const unminifiedJs= `
...
const tests: Array<RuleTest> = [
    {
        name: 'This test should pass because content is minified',
        serverConfig: generateHTMLPage(generateScriptTag(minifiedJs))
    },
    {
        name: 'This test should fail because content is unminified',
        reports: [{ message: expectedMessageFromRule }],
        serverConfig: generateHTMLPage(generateScriptTag(unminifiedJs))
    }
];
const testsWithThreshold: Array<RuleTest> = [
    {
        name: 'This test should pass because threshold is 100',
        serverConfig: generateHTMLPage(generateScriptTag(unminifiedJs))
    }
];
 ...
 */
ruleRunner.testRule(ruleName, testsWithThreshold, { parsers: ['javascript'], ruleOptions: { threshold: 100 }});
