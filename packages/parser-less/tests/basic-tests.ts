...
const mockCSS = async (sandbox: sinon.SinonSandbox, type: string, code: string) => {
    ...
    const element = mockStyleElement(type, code);
...
test('We should provide a correct AST when parsing LESS.', async (t) => {
    const code = `
@primary-color: #fff;
...
`;
    ...
    const engineEmitAsyncSpy = await mockCSS(sandbox, 'text/less', code);
...
test('We should provide an AST when parsing empty LESS.', async (t) => {
    const code = '';
    const sandbox = sinon.createSandbox();
    const engineEmitAsyncSpy = await mockCSS(sandbox, 'text/x-less', code);
