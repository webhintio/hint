...
test.serial('If we receive a valid json with a valid name, it should emit the event parse::end::package-json', async (t) => {
    await t.context.engine.emitAsync('fetch::end::json', {
        resource: url.format(getAsUri(validPath)!),
        response: { body: { content: JSON.stringify(validJSON) } }
    });
...
test.serial(`If package.json contains an invalid schema, it should emit the 'parse::error::package-json::schema' event`, async (t) => {
    await t.context.engine.emitAsync('fetch::end::json', {
        resource: 'package.json',
        response: { body: { content: invalidJSON } }
    });
...
test.serial(`If package.json contains an invalid schema, it should emit the an errors array containing the errors `, async (t) => {
    await t.context.engine.emitAsync('fetch::end::json', {
        resource: 'package.json',
        response: { body: { content: invalidJSON } }
    });
...
test.serial(`If package.json contains an invalid schema, it should emit a prettified version of the errors `, async (t) => {
    await t.context.engine.emitAsync('fetch::end::json', {
        resource: 'package.json',
        response: { body: { content: invalidJSON } }
    });
