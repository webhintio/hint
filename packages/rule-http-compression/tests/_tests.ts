...
const testsForBrotli: Array<RuleTest> = [
    ...
    },
    {
        ...
        reports: [{ message: `Couldn't fetch content when Brotli compression is requested` }],
        serverConfig: createBrotliServerConfig({
            scriptFileContent: scriptFile.brotli,
...
const testsForGzipZopfli = (https: boolean = false): Array<RuleTest> => {
    return [
        ...
        },
        {
            ...
            name: `If a request throws an exception, if should be managed and report an error`,
            reports: [{ message: 'Error fetching gzip content' }],
            serverConfig: createGzipZopfliServerConfig(
                {
                    request: { headers: { 'Accept-Encoding': 'gzip' } },
                    scriptFileContent: scriptFile.zopfli
...
const testsForNoCompression = (https: boolean = false): Array<RuleTest> => {
    return [
        ...
        },
        {
            ...
            name: `If a request throws an exception, if should be managed and report an error`,
            reports: [{ message: 'Error fetching uncompressed content' }],
            serverConfig: createGzipZopfliServerConfig(
                {
                    faviconFileContent: faviconFile.original,
                    faviconFileHeaders: { 'Content-Encoding': null },
