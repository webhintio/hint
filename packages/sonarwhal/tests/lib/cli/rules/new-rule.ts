...
test.serial('It creates a rule if the option multiple rules is false', async (t) => {
    ...
    t.true(fsExtraCopyStub.args[0][0].endsWith('files'), 'Unexpected path for external files');
...
test.serial('It creates a package with multiple rules', async (t) => {
    ...
    t.true(fsExtraCopyStub.args[0][0].endsWith('files'), 'Unexpected path for common files');
