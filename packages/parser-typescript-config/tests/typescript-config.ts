...
test('If any file is parsed, it should emit a `parser::typescript-config::error::not-found` error', async (t) => {
    ...
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::typescript-config::error::not-found');
...
test('If the file contains an invalid json, it should fail', async (t) => {
    ...
    // 2 times, the previous call, and the expected call.
    t.true(t.context.sonarwhal.emitAsync.calledTwice);
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::typescript-config::error::json');
...
test('If the file contains a valid json with an invalid schema, it should fail', async (t) => {
    ...
    // 2 times, the previous call, and the expected call.
    t.true(t.context.sonarwhal.emitAsync.calledTwice);
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::typescript-config::error::schema');
    t.is(t.context.sonarwhal.emitAsync.args[1][1].errors[0].message, 'should NOT have additional properties');

    sandbox.restore();
});

test('If we receive a valid json with a valid name, it should emit the event parser::typescript-config::end', async (t) => {
    const sandbox = sinon.sandbox.create();
    ...
    t.is(t.context.sonarwhal.emitAsync.args[1][0], 'parser::typescript-config::end');
