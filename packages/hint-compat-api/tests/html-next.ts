...
const elementVersionAddedFalse: HintTest[] = [
    {
        ...
        reports: [{ message: 'blink element is not supported on chrome browser.', position: { column: 1, line: 1}}],
...
const elementVersionAddedFalseForMultipleBrowsers: HintTest[] = [
    {
        ...
        reports: [{ message: 'blink element is not supported on chrome, edge, ie browsers.', position: { column: 1, line: 1}}],
        serverConfig: generateHTMLConfig('blink')
    }
];

hintRunner.testHint(hintPath, elementVersionAddedFalseForMultipleBrowsers, { browserslist: ['chrome 43', 'last 2 Edge versions', 'last 2 ie versions'], parsers: ['html']});
